function download_backups(event){
    for(let i = 0; i < event.data.backups.length; i++){
        event.data.backups[i] = event.data.backups[i] + "\n"
    }
    var newBlob = new Blob(event.data.backups, {type: "text/plain;charset=utf-8"});
    saveAs(newBlob, event.data.website + "_backup_codes.txt");
}
